...
public class DsOsmDestinationResolver implements DsDestinationResolver {

  protected static final RetrySettings
      RETRY_SETTINGS =
      RetrySettings.newBuilder().setMaxAttempts(6).setInitialRetryDelay(Duration.ofSeconds(10L))
          .setMaxRetryDelay(Duration.ofSeconds(32L)).setRetryDelayMultiplier(2.0D)
          .setTotalTimeout(Duration.ofSeconds(50L)).setInitialRpcTimeout(Duration.ofSeconds(50L))
          .setRpcTimeoutMultiplier(1.0D).setMaxRpcTimeout(Duration.ofSeconds(50L)).build();
  protected static final TransportOptions
      TRANSPORT_OPTIONS = HttpTransportOptions.newBuilder().setReadTimeout(30000).build();
  ...
  public DsDestinationResolution resolve(Destination destination) {
    ...
    datastoreCache.computeIfAbsent(partitionId, partition -> {
    ...
    return DsDestinationResolution.builder()
        .projectId(datastoreCache.get(partitionId).getOptions().getProjectId())
  ...
  public void close() throws IOException {
    //Do nothing because datastore does not require special closing procedures.
  }
}
